# Component and format notes (README snapshot, 0.67.0)

This is the body of the README as it stood at 0.67.0, when the README was reduced to contributor matters and
user documentation moved to [ufoathome.org](https://ufoathome.org/docs/). It is **not maintained**: it is kept
because some of it is not on the site yet, and some of it is design rationale a contributor may want.

- User-facing facts still missing from the site (component members and behaviours, format fields such as
  `timeZone`, decor lights' `perMinute`/`dutyCycle`/`phase`, decor `sizeM`/`model`, `deep-star-catalog-src`,
  sound keyframe blending...) belong in `site/content/Docs*Page.ts`, in both languages. Move them there and
  delete them here.
- Some statements may already be stale (e.g. `caseId`, the `instrument` values): check against the code first.
- Design rationale and implementation notes (why a choice was made, which class does what) belong next to the
...
fields the toolbar currently holds (see [Layout](../README.md#layout)). This absorbs `<rr0-scene>`'s own
