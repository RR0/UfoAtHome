...
and `scripts/data/stellarium-satellites.json` for magnitudes, and writes `dist-tle/` (gitignored):
...
838 MB of snapshots become 44 MB. The dev server serves it at `/tle/`, and `build:site` copies it
...
npm run build:tle              # rebuild dist-tle/ from a local copy of the orbital element archive
```
