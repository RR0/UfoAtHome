...
draws — the things that were genuinely up there and are genuinely mistaken for something else. Two so far, chosen
because their record is complete for every date this project can reconstruct, with no lookup, no key and no
coverage floor:
...
All three appear in the recorder's read-only "Sky:" line, with a button to turn the witness toward
