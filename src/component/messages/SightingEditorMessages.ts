...
export interface SightingEditorMessages extends SightingLabels {
  ...
  /** The slider that decides how far along its line of sight a shape is DRAWN — a reader's
   * hypothesis, never the recording's (see PhenomenonDepth and SceneElement.setDistanceHypothesis). */
  distanceHypothesis: string
  /** The button that withdraws it. */
  ...
  /** What was written into the editor, {count} fields — a count and not just "done", because a
   * draft states only what the account states and the number is how much that turned out to be. */
