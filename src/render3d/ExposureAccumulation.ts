...
export class ExposureAccumulation {
  /** Where one instant lands, straight out of the pass chain: the scene's luminance and its
   * overlays, neither finished (see UnfinishedFrame). */
  private readonly frame: UnfinishedFrame
  /** The running sums of every instant so far — the film itself, and what was laid over it. */
  private readonly film: UnfinishedFrame
  ...
  constructor(width: number, height: number) {
    ...
    this.frame = { scene: target(), overlay: target() }
    this.film = { scene: target(), overlay: target() }
    const vertexShader = `
      varying vec2 vUv;
      void main() {
    ...
    this.addMaterial = new ShaderMaterial({
      uniforms: { uSource: { value: this.frame.scene.texture }, uShare: { value: 1 } },
      vertexShader,
      fragmentShader: `
        ...
        void main() {
          // Every channel, coverage included: the overlay's is averaged over the pose like its colour.
          gl_FragColor = texture2D(uSource, vUv) * uShare;
        }
      `,
      // The whole mechanism, in one line: the GPU's own adder is the shutter staying open. A pure
      // sum — three's AdditiveBlending would multiply the colour by the coverage first.
    ...
    this.developMaterial = new ShaderMaterial({
      ...
      fragmentShader: `
        ...
        uniform float uGain;
        void main() {
          gl_FragColor = vec4(finish(texture2D(uFilm, vUv).rgb * uGain, texture2D(uOverlay, vUv) * uGain), 1.0);
        }
      `,
      blending: NoBlending,
      depthTest: false,
  ...
  get instantTarget(): UnfinishedFrame {
    return this.frame
  }
  ...
  open(renderer: WebGLRenderer): void {
    ...
    for (const layer of [this.film.scene, this.film.overlay]) {
  ...
  add(renderer: WebGLRenderer, share: number): void {
    ...
    for (const [from, onto] of [[this.frame.scene, this.film.scene], [this.frame.overlay, this.film.overlay]] as const) {
      this.addMaterial.uniforms.uSource.value = from.texture
  ...
  resize(width: number, height: number): void {
    this.width = Math.max(1, width)
    this.height = Math.max(1, height)
    for (const layer of [this.frame.scene, this.frame.overlay, this.film.scene, this.film.overlay]) layer.setSize(this.width, this.height)
  }

  dispose(): void {
    for (const layer of [this.frame.scene, this.frame.overlay, this.film.scene, this.film.overlay]) layer.dispose()
