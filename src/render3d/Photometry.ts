...
export class Photometry {
  ...
  static shown(colour: readonly [number, number, number], luminanceCdM2: number, display?: LuminanceDisplay): [number, number, number] {
    ...
    // How light a white of that luminance is on the screen: its response, shown as a lightness.
    const brightness = EyeAdaptation.fromLightness(EyeAdaptation.respond(Photometry.luminanceOf(white)))
    const peak = Math.max(colour[0], colour[1], colour[2]) || 1
    const hue: [number, number, number] = [colour[0] / peak, colour[1] / peak, colour[2] / peak]
    // The frame's last pass keeps a colour's chromaticity and shows the response to its luminance
    // as a lightness; so the light drawn is the one that comes out as this hue at that brightness.
    const luminance = Photometry.luminanceOf(hue)
    if (!(luminance > 0) || !(brightness > 0)) return [0, 0, 0]
    const scale = EyeAdaptation.relativeOfResponse(EyeAdaptation.lightnessOf(luminance * brightness)) / luminance
