...
/** One recorded position along the observer's own path, with the direction they faced there when
 * the recording says (see ObserverPose.headingDeg — undefined is "nobody wrote it down", never
 * north). */
export interface ObserverPathPoint {
  t: number
  lat: number
  lng: number
  headingDeg?: number
...
/**
 * Where the observer stood, over the whole recording, as ground truth rather than as a camera.
 *
 * The same `observerTrack` the scene reads to place its camera, asked a different question: not
 * "what does the observer see now" but "where were they, and where had they been". A observer who
 ...
 * before, since holding a position is a claim that the observer stayed put and the file did not
 * make it.
 */
export class ObserverPath {
  /** Under this, the observer is standing still as far as any map at this scale can show — a GPS fix
   * is not better than this, and neither is a observer pointing at a spot on a map decades later. */
  private static readonly STATIONARY_M = 5

  private constructor(readonly points: ReadonlyArray<ObserverPathPoint>) {}
   ...
   */
  static of(sighting: Sighting): ObserverPath | undefined {
    const points: ObserverPathPoint[] = []
  ...
  get moved(): boolean {
    return this.spanM > ObserverPath.STATIONARY_M
  }
  ...
  /** The midpoint of that box — where the map is centred, so a observer who walked across it stays
   ...
   *
   * The floor matters as much as the margin. A observer who never moved has a span of zero, and a
   * map zoomed to a zero-metre box is a map of one blade of grass; a observer who covered eleven
